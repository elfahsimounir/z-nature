...
async function generateUniqueSlugForOffer(desired: string, currentId?: string) {
  let slug = desired;
  let i = 1;
  // Loop until a unique slug is found (ignoring current record on update)
  // For SQLite this is fine; for large datasets consider optimizing.
...
export async function POST(req: Request) {
  try {
    ...
    if (desktopImageFile) {
      ...
      const key = `offers/${crypto.randomUUID()}.${ext}`;
    ...
    const slug = await generateUniqueSlugForOffer(product.slug);
...
export async function PATCH(req: Request) {
  try {
    ...
    const slug = await generateUniqueSlugForOffer(desired, id);
    ...
    console.error("PATCH /api/offer error:", error);
    return new Response(JSON.stringify({ error: error.message || "An unknown error occurred" }), { status: 500 });
  }
}
...
export async function DELETE(req: Request) {
  try {
    ...
    console.error('DELETE /api/offer error:', error);
    return new Response(JSON.stringify({ error: error.message || 'An unknown error occurred' }), { status: 500 });
  }
}
