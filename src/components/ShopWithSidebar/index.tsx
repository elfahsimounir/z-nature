...
import HashtagsDropDown from "./HashtagsDropDown";
import PriceDropdown from "./PriceDropdown";
import SingleGridItem from "../Shop/SingleGridItem";
import SingleListItem from "../Shop/SingleListItem";
import { useRouter } from "next/navigation";
import CustomSelect from "./CustomSelect";
import { ArrowUpWideNarrow, ChevronLeft, ChevronRight, ListRestart } from "lucide-react";
import { Button } from "../ui/button";

const staticData = {
  sortingOptions: [
    { label: "Derniers Produits", value: "new" },
    { label: "Anciens Produits", value: "old" },
    { label: "Prix : Du Moins Cher au Plus Cher", value: "price-asc" },
    { label: "Prix : Du Plus Cher au Moins Cher", value: "price-desc" },
...
const ShopWithSidebar = ({ data, categories, brands, hashtags, filters }) => {
  const [filteredData, setFilteredData] = useState(data);
  const [dynamicBrands, setDynamicBrands] = useState(brands);
  const [dynamicHashtags, setDynamicHashtags] = useState(hashtags);
  const [selectedCategory, setSelectedCategory] = useState(filters.category || null); // Default category
  const [selectedBrand, setSelectedBrand] = useState(filters.brand || null); // Default brand
  const [selectedHashtags, setSelectedHashtags] = useState(filters.hashtags || null); // Default hashtags
  ...
  useEffect(() => {
    ...
    if (filters.category && categories && categories.length > 0) {
      const selected = categories.find((c:any) => c.name === filters.category);
      if (selected) {
        // Build descendant name set (including self)
        ...
        filtered = filtered.filter((product) => names.has(product.category.name));
      }
    }
    ...
    if (filters.hashtags) {
      const hashtagsArray = filters.hashtags.split(",");
      filtered = filtered.filter((product) =>
        product.hashtags.some((hashtag) => hashtagsArray.includes(hashtag.name))
      );
    }
    ...
    // Update dynamic brands and hashtags based on filtered products
    const updatedBrands = brands.filter((brand) =>
      filtered.some((product) => product.brand.name === brand.name)
    );
    const updatedHashtags = hashtags.filter((hashtag) =>
      filtered.some((product) =>
        product.hashtags.some((productHashtag) => productHashtag.name === hashtag.name)
      )
    );

    setDynamicBrands(updatedBrands);
    setDynamicHashtags(updatedHashtags);
  }, [filters, data, brands, hashtags,selectedBrand,selectedCategory,selectedHashtags]);
  ...
  const handleFilterChange = (newFilters) => {
    ...
    Object.keys(newFilters).forEach((key) => {
      if (newFilters[key] === null || newFilters[key] === undefined || newFilters[key] === "") {
        query.delete(key); // Remove the parameter if its value is null, undefined, or empty
      } else {
        query.set(key, newFilters[key]); // Otherwise, update the parameter
      }
    });
  ...
  const handleSortChange = (sortOption) => {
    handleFilterChange({ ...filters, orderby: sortOption });
  };
  ...
  // Pagination logic
  const indexOfLastItem = currentPage * staticData.itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - staticData.itemsPerPage;
  const currentItems = filteredData.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(filteredData.length / staticData.itemsPerPage);
  return (
    <section className="">
      <Breadcrumb
      ...
      />
      <div className="overflow-hidden relative pb-10 pt-5 lg:pt-9 xl:pt-10 bg-[#f3f4f6]">
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="flex gap-7.5">
            {/* Sidebar */}
            {openSideBar&&
            <div className="sidebar-content rounded-md overflow-hidden shadow-1 bg-white fixed xl:z-1 z-[9999] left-0 top-0 xl:translate-x-0 lg:static max-w-[310px] xl:max-w-[270px] w-full ease-out duration-200">
            <form onSubmit={(e) => e.preventDefault()} className="bg-white h-screen ">
              <div className="flex flex-col gap-6 ">
              <div className="bg-white rounded-lg py-4">
                  <div className="flex items-center justify-between px-5">
                    <p>{staticData.labels.filter.title}</p>
                    <Button  onClick={handleResetFilters} variant={'ghost'}><ListRestart strokeWidth={1.5} size={24} /></Button>
                    <Button
                    className="lg:hidden"
                  ...
                  <div className="flex w-full py-5 flex-col h-[90vh] lg:h-full overflow-y-auto">
                <CategoryDropdown
                  title={staticData.labels.category}
                ...
                />
                <BrandsDropdown
                  title={staticData.labels.brand}
                  brands={dynamicBrands}
                  selectedBrand={selectedBrand} // Pass the selected brand
                  onChange={(brand) => {
                    const newBrand = selectedBrand === brand ? null : brand;
                    setSelectedBrand(newBrand); // Update local state
                    handleFilterChange({ brand: newBrand });
                  }}
                />
                <HashtagsDropDown
                  title={staticData.labels.hashtags}
                  selectedHashtags={selectedHashtags}
                  hashtags={dynamicHashtags}
                  onChange={(hashtag) => {
                    const hashtagsArray = filters.hashtags ? filters.hashtags.split(",") : [];
                    const newHashtags = hashtagsArray.includes(hashtag)
                      ? hashtagsArray.filter((h) => h !== hashtag).join(",")
                      : [...hashtagsArray, hashtag].join(",");
                    handleFilterChange({ hashtags: newHashtags });
                  }}
                />
                <PriceDropdown
                  title={staticData.labels.price}
                  onChange={(priceRange) =>
                    handleFilterChange({ ...filters, price_min: priceRange.from, price_max: priceRange.to })
                  }
                />
                </div>
                </div>
            ...
            <div className="xl:max-w-[870px] w-full">
              <div className="rounded-lg bg-white shadow-1 pl-3 pr-2.5 py-2.5 mb-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    <CustomSelect
                      options={staticData.sortingOptions}
                      onChange={(selectedOption) => handleSortChange(selectedOption.value)} // Ensure onChange works
                    />
                        <p className="hidden lg:block text-sm">
                      {staticData.labels.pagination.showing} <span className="text-dark">{currentItems.length}   {staticData.labels.pagination.of} {data.length}</span>   {staticData.labels.pagination.products} 
                    </p>
                  </div>
                  ...
                  <button
                      ...
                      className={`${"text-dark bg-gray-1 border-gray-3"
                      } flex items-center justify-center  lg:w-10.5 lg:h-9 w-7 h-7 rounded-[5px] border ease-out duration-200 hover:bg-gray-2`}
                    >
                   <ArrowUpWideNarrow strokeWidth={1.5} size={18} />
                    </button>
                    <button
                      ...
                      className={`${
                        productStyle === "grid"
                          ? "bg-gray-1 border text-primary border-primary"
              ...
              <div className={productStyle === "grid" ? "grid grid-cols-1 md:grid-cols-3 gap-4" : "flex flex-col gap-4"}>
