...
type Product = { id: string; name: string; image: string; slug: string };

type Entity = "banner" | "promotion" | "offer" | "publication";
...
export default function EntityPage({ entity, title }: { entity: Entity; title: string }) {
  const [rows, setRows] = useState<FormDataT[]>([]);
  const [defaultValues, setDefaultValues] = useState<Partial<FormDataT>>({});
  ...
  const handleEdit = (row: any) => {
    ...
    setDefaultValues({
      ...
      desktopImage: row.desktopImage || row.image || null,
      mobileImage: row.mobileImage || row.image || null,
    });
    setSelectedProduct(product);
    setSelectingProduct(false);
    setOpenModal(true);
  ...
  const handleSubmit = async (data: any) => {
    ...
    try {
      const res = await fetch(`/api/${entity}`, { method, body: formData });
      if (res.ok) {
        const updated = res.status !== 204 ? await res.json() : null;
        setRows((prev) =>
          defaultValues.id
        ...
        showNotification(`Échec de l'enregistrement : ${error.error}` , "error");
      }
    } catch (err) {
  ...
  const handleDelete = async (selectedItems: FormDataT[]) => {
    const ids = selectedItems.map((item) => item.id).join("&id=");
    try {
      const res = await fetch(`/api/${entity}?id=${ids}`, { method: "DELETE" });
      if (res.ok) {
        setRows((prev) => prev.filter((row) => !selectedItems.some((i) => i.id === row.id)));
        showNotification("Éléments sélectionnés supprimés avec succès", "success");
      } else {
        const error = await res.json();
        showNotification(`Échec de la suppression : ${error.error}`, "error");
      }
    } catch (err) {
  ...
  const breadcrumb = { title, pages: ["Admin", `/ ${entity}`] };
      ...
      <div className="relative">
        ...
        <DataTable
          ...
          columns={[
            ...
            { accessorKey: "description", header: "Description", cell: ({ row }) => <div className="text-start">{row.getValue("description")}</div> },
            {
              accessorKey: "desktopImage",
              header: "Desktop",
              cell: ({ row }: { row: any }) =>
                row.original.desktopImage || row.original.image ? (
                  <Image src={(row.original.desktopImage || row.original.image) as string} width={64} height={64} alt={row.original.name || "image"} className="w-auto h-16 object-cover" />
                ) : (
            ...
            },
            {
              ...
              cell: ({ row }: { row: any }) =>
                row.original.mobileImage || row.original.image ? (
                  <Image src={(row.original.mobileImage || row.original.image) as string} width={64} height={64} alt={row.original.name || "image"} className="w-auto h-16 object-cover" />
                ) : (
                  <div className="text-gray-500">Aucune image</div>
                ),
            },
        ...
        {selectingProduct && (
          <div className="bg-white p-4 rounded shadow-sm border z-99 absolute w-[500px] top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2">
            <div className="flex justify-between items-center mb-1">
              ...
              <Button variant="ghost" onClick={() => setSelectingProduct(false)} className="text-dark">
                Cancel
              </Button>
            </div>
            <input
              ...
              {products.map((product) => (
                ...
                >
                  <div className="w-15 flex justify-center">
                    <Image src={product.image || "/placeholder.png"} alt={product.name} width={100} height={100} className="object-cover mb-2 h-10 w-auto" />
                  </div>
