...
const SingleListItem = ({ item }: { item: any }) => {
 ...
 const { showNotification } = useNotification();
  ...
  return (
    <div className="group rounded-lg bg-white relative shadow-1">

      <div className="flex">
        <div className="shadow-list relative overflow-hidden flex items-center justify-center max-w-[270px] w-full sm:min-h-[270px] p-4">
          <Image src={item.images[0].url} alt={item.name} width={250} height={250} />
        ...
        <div className="w-full flex flex-col gap-5 sm:flex-row sm:items-center justify-center sm:justify-between py-5 px-4 sm:px-7.5 lg:pl-11 lg:pr-12">
          <div>
            ...
            {((item.discount / (item.price as number)) * 100) > 5 &&
            ...
            <Link href={`/shop/${formatSlug(item.slug as string)}`}>{item.name} </Link>
              <span className="text-xs text-dark-4"> {item.description}</span>
            </h3>
            <div className="my-2">
            ...
            <span className="flex items-center gap-2 font-medium text-lg">
              <span className="text-dark">{item.price as any - item.discount} DH</span>
