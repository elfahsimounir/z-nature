...
import IsNew from "../Common/IsNew";
  ...
  const handleAddToCart = () => {
    dispatch(
      addItemToCart({
        ...
        quantity: 1,
        images: item.images as any , // Use the first image URL to match the expected type
      })
    );
    showNotification(
  ...
  return (
    <div className="group">
      ...
      <IsNew inew={item.isNew}/>
       <Image src={item.images[0].url} alt={item.name} width={250} height={250}  className="max-w-[200px] h-auto"/>
...
<div className="flex flex-col h-full">
    ...
    {/* <div className="flex items-center gap-2.5 mb-2">
      ...
      <h3 className="font-medium text-dark ease-out duration-200 mb-1.5">
      <Link href={`/shop/${formatSlug(item.slug as string)}`}>{item.name} </Link>
      </h3>
     <div>
     <span className="flex gap-2 font-medium text-lg items-center">
