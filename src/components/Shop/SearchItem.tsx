...
const SearchItem = ({ item,name }: { item: any ,name?:ReactNode}) => {
  const { openModal } = useModalContext();
  const {closeSearchModal  }=useCartModalContext()
  ...
  const handleAddToCart = () => {
    dispatch(
      addItemToCart({
        ...
        quantity: 1,
        images: item.images as any, // Use the first image URL as a string
      })
    );
  };
  ...
  return (
    <div className="group  bg-white relative shadow-1 border-b border-gray-1">
      <div className="flex">
        <div className="shadow-list relative overflow-hidden flex items-center justify-center max-w-[270px] w-full py-4">
          <Image src={item.images[0].url} alt={item.name} width={100} height={100} />
        ...
        <div className="w-full flex flex-col gap-2 sm:flex-row sm:items-center justify-center sm:justify-between py-4 px-4 sm:px-7.5 lg:pl-11 lg:pr-12">
          <div>
            <div className="flex items-center gap-2.5 mb-2">
            {item.isNew && (
              <span className="inline-block bg-secondary text-white text-custom-sm font-semibold px-2.5 rounded-r-full ">New</span>
            )}

            {Math.round((item.discount / (item.price as number)) * 100) > 5 &&
              <span className="text-secondary">
                {Math.round((item.discount / (item.price as number)) * 100)}% OFF
              </span>
            }
            </div>
           
            <h3 className="font-medium text-dark ease-out duration-200 flex flex-col   mb-1.5">
            <Link
            onClick={closeSearchModal}
            href={`/shop/${formatSlug(item.slug as string)}`}>{name}</Link>
            ...
            <span className="flex items-center gap-2 font-medium text-lg">
              <span className="text-primary">{item.price as any - item.discount} DH</span>

              <span className="text-dark-4 line-through">{item.price} DH</span>
            </span>
          </div>
...
const Button = ({ children, onClick }) => {
 return <button
  ...
  className="flex items-center justify-center w-9 h-9 rounded-[5px] shadow-1 ease-out duration-200 text-dark bg-white hover:text-secondary"
>
{children}
</button>
}