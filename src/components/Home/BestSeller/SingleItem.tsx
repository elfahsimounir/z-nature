...
import IsNew from "@/components/Common/IsNew";
import StockToggler from "@/components/ui/stock-toggler";
...
const SingleItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group bg-[#F6F7FB]  overflow-hidden rounded-lg border border-gray-2">
      <div className="relative min-h-[403px]">
      <IsNew inew={item.isNew} />
        <div className="text-center px-4 py-7.5">
      
          <h3 className="font-medium text-dark ease-out duration-200 e mb-1.5">
            <Link href={`shop/${formatSlug(item.slug)}`}> {item.name} </Link>
          </h3>
          <span className="flex items-center justify-center gap-2 font-medium text-lg">
            <span className="text-dark">{(item.price ?? 0) - (item.discount ?? 0)}DH</span>
            <span className="text-dark-4 line-through text-sm">{item.price}DH</span>
          </span>
          <div className="mb-2 flex gap-2 items-center justify-center ">
       <StockToggler stock={item.stock} />
       <div className="flex items-center justify-center gap-2.5 border-l pl-2">
            <Starts count={5} />
            <p className="text-custom-sm">({(item as any ).reviews.length})</p>
          </div>
       </div>
        </div>

        <div className="flex justify-center items-center w-auto h-[252px] p-3 relative">
          <Image src={(item as any).images[0].url} alt="" width={280} height={280}  className="!h-full w-auto"/>
        </div>
