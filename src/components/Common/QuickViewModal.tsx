...
const QuickViewModal = () => {
  ...
  function checkWishlist(name: string):boolean {
    return wishlistItems.some((item:any) => item.name === name);
  }

  const dispatch = useDispatch<AppDispatch>();

  // get the product data
  const product:any = useAppSelector((state) => state.quickViewReducer.value);
  ...
  const handlePreviewSlider = () => {
    dispatch(updateproductDetails(product));
     setActivePreview(product.images[0]);
    openPreviewModal();
  };
  ...
  }, [isModalOpen, closeModal, product.images]);

  return (
    <div
      className={`${
        isModalOpen ? "z-[99999999]" : "hidden"
        ...
        <div className="w-full max-w-[1100px] rounded-xl shadow-3 bg-white p-7.5 relative modal-content">
          ...
          <div className="flex flex-wrap items-center gap-12.5">
            <div className="max-w-[526px] w-full">
              <div className="flex gap-5">
                <div className="flex flex-col gap-5">
                  {(product.images as any)?.map((img, key) => (
                    <button
                      onClick={() => setActivePreview(img)}
                      key={key}
                      className={`flex items-center justify-center w-20 h-20 overflow-hidden rounded-lg bg-gray-1 ease-out duration-200 border hover:border-primary ${
                        activePreview?.url === img.url && " border-gray-6"
                      }`}
                    >
                      <Image
                        src={img.url || ""}
            ...
            <div className="max-w-[445px] w-full">
              {((product.discount ?? 0) / (product.price ?? 1))>5&&
              <span className="inline-block text-custom-xs font-medium text-white py-1 px-3 bg-secondary rounded-r-xl mb-6.5">
              SALE  {Math.round(((product.discount ?? 0) / (product.price ?? 1)) * 100)}%  OFF
            </span>
              }
              ...
              <div className="flex flex-wrap items-center gap-5 mb-6">
                ...
                <StockToggler stock={product.stock}/>
              </div>
              ...
              <div className="flex flex-wrap justify-between gap-5 mt-6 mb-7.5">
                <div>
                  ...
                  <span className="flex items-center gap-2">
                    <span className="font-medium text-primary text-xl xl:text-heading-4">
                      {(product.price ?? 0) - (product.discount ?? 0)} DH
                    </span>
                    <span className=" text-dark-4 text-lg xl:text-2xl line-through">
                      {product.price} DH
                    </span>
                  </span>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-4">
                <Button
                  ...
                  className={`inline-flex font-medium hover:bg-primary/30 text-primary py-3 px-7 rounded-md ease-out duration-200 bg-primary/10`}
                >
                  Add to Cart
                </Button>

                <Button
                  className={`inline-flex font-medium hover:bg-secondary/30 text-dark py-3 px-7 rounded-md ease-out duration-200 bg-secondary/10`}
                    onClick={() => handleItemToWishList()}
                    >
                  <svg

                    className={`fill-current ${checkWishlist(product.name) ? "text-secondary" : "text-dark"} `}
                    width="20"
                    height="20"
                    viewBox="0 0 20 20"
                    fill="none"
