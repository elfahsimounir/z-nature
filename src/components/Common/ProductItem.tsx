  ...
  return (
    <div className="group">
      ...
      <IsNew inew={item.isNew} />
        <Image src={item?.images[0].url} alt={item.name} width={250} height={250} className="max-h-[270px] w-auto" />
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          ...
          >
            <svg
              className={`fill-current relative ${checkWishlist(item.name) ? "text-secondary" : ""}`}
              width="16"
              height="16"
              viewBox="0 0 16 16"
              fill="none"
            ...
            >
              ...
              fill={checkWishlist(item.name) ? "currentColor" : ""}
              />
            </svg>
          </button>
        </div>
      ...
      <div className="flex items-center gap-2.5 mb-2">
       <Starts count={5} />
        ...
        <p className="text-xs">({item.reviews? item.reviews.length:0})</p>
      </div>
       <StockToggler stock={item.stock} />
      <h3
        className="font-medium text-dark ease-out duration-200 mb-1.5"
...
<Link href={`/shop/${formatSlug(item.slug)}`}> {item.name} </Link>
      ...
      <span className="flex items-center gap-2 font-medium text-lg">
        <span className="text-dark">{+item.price - +item.discount} DH</span>
